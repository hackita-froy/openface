...
def walkAndCalcReps(root_dir, dscrFun, suffix):
    ...
    """
    processedFname = os.path.join(root_dir,'processed' + suffix + '.pkl')
    repFname = os.path.join(root_dir,'repsDBase' + suffix +  '.pkl')
    repFnameOldFmt = os.path.join(root_dir,'repsDBaseOldFmt' + suffix + '.pkl')
    failFname = os.path.join(root_dir,'failDBase' + suffix + '.pkl')
    repFnameJson = os.path.join(root_dir,'repsDBase' + suffix + '.json')
...
if __name__ == "__main__":
    ...
    parser.add_argument('--descriptor', type=str,
                        ...
                        default='openface',
                        choices=['openface', 'vgg'])

    args = parser.parse_args()

    dscr = args.descriptor

    if type(dscr) is list:
        dscr = dscr[0]


    if dscr.lower()=='openface':
        dscrFun = lambda fname: getImgRep(getFaceRep.getRep, fname)
        suffix = ''
    elif dscr.lower()=='vgg':
        print "ain't implemented yet"
        raise
        dscrFun = getFaceRepVGG.getRep
        suffix = '_vgg'
    else:
    ...
    walkAndCalcReps(args.root_dir[0], dscrFun, suffix)
